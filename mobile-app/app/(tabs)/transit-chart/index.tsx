...
export default function TransitChartPersonList() {
    ...
    <LinearGradient colors={['#0A0A0F', '#0E0818']} style={styles.gradient}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Transit Harita</Text>
          <Text style={styles.subtitle}>Güncel transit hareketlerini incele</Text>
        </View>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <PersonCard
                person={item}
                onPress={() => router.push(`/(tabs)/transit-chart/${item.id}`)}
            ...
            ListEmptyComponent={
              <View style={styles.empty}>
                ...
                <Text style={styles.emptySub}>Kişi eklemek için aşağıdaki butona bas</Text>
              </View>
            }
          />
        )}
...
const styles = StyleSheet.create({
  ...
  searchWrap: {
    ...
    paddingHorizontal: 12, borderWidth: 1, borderColor: colors.cardBorder,
  },
